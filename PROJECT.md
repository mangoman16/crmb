...
| A JavaScript framework | Server-rendered HTML plus ~175 lines of JS does the job. A build step is a thing that breaks between her and her data. |
...
| | |
|---|---|
| Application code | ~8,320 lines PHP in `app/`, 2,419 lines of views, 784 lines CSS, 175 lines JS |
| Schema | 42 tables, 18 migrations |
| Configuration | 52 settings, each declared once with a type and a default |
| Tests | 22 suites, no database server needed, runs in seconds |
| Dependencies | 2: phpmailer 7.1.1, bacon/bacon-qr-code 3.1.1 (both pinned in `composer.lock`) |
| Undo coverage | 13 tables versioned with one-click revert |

Those numbers were counted on 2026-09-22, against 0.6.0. They have gone stale
twice; re-measure them when you cut a release rather than trusting the row.
...
- All eighteen migrations apply, including the two
...
- The whole test suite passes on the real engine. What that run cannot reach it
  ...
  migrations 015 and 016 is still checked on SQLite only.
...
|---|---|---|---|
| 1.1 | ~~Run migrations and the suite against a real engine~~ **Done on MariaDB 10.11.14** | The dialect was unproven | ✅ Eighteen migrations apply and the suite passes there; the run names what it could not cover. Repeat with `tests/mariadb-local.sh`. Remaining: the same on MySQL 8.0, if that is the target |
